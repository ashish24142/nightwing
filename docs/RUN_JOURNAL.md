...
- **Model:** Qwen3-14B (Apache-2.0) + bf16 LoRA (r=32, alpha =64, dropout 0.05, all
...
- **Contamination:** `verify_split.py` gates every training launch, train/test
  overlap 0, dev overlap test 0, dev overlap train-data 0, asserted in code each time.
...
| | |
|---|---|
| Pod | RunPod Secure Cloud, 1x A100 SXM 80 GB, 24 vCPU, 117 GB RAM, 210 GB disk, $1.52/hr all-in |
| Image | runpod/pytorch 2.4 template (py3.11, cuda 12.4); torch 2.10.0+cu128 at run time |
| Stack | transformers + peft + trl (bf16 LoRA path); **no Unsloth** (see section 3) |
...
3., because the sentinel came from **Unsloth's tokenizer patching** clashing with
   ...
   Cost: ~1.5x slower steps. Two runs were lost to this pair of failures (~45 min).
...
| GPU pod (~33 h x $1.52: train 13.1 h, sweep 2.1 h, test eval 11.5 h, setup/babysitting/idle rest) | ~$50 |
